<template>
    ...
    <Head>
        <title>Jenis Plat - BSS Parking</title>
    </Head>
                ...
                <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
                    <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                        <ol class="breadcrumb text-muted fs-6 fw-semibold mb-3">
                            ...
                            <li class="breadcrumb-item pe-3 text-muted">Jenis Plat</li>
                        </ol>
                        <h1 class="page-heading d-flex text-dark fw-bold flex-column justify-content-center my-0">Jenis Plat</h1>
                    </div>
                </div>
            </div>
                ...
                <div id="kt_app_content_container" class="app-container container-xxl px-0">
                    <div class="card border border-gray-300">
                        <div class="card-header border-0 px-8 py-5 d-flex flex-wrap gap-3">
                            <form @submit.prevent="handleSearch"
                                ...
                                <input type="text" class="form-control form-control-solid ps-13 fs-5"
                                    placeholder="Cari jenis plat disini..." v-model="search" />
                            </form>
                        ...
                        <div class="card-body px-0 pt-0 pb-5">
                            <div class="table-responsive">
                                <table id="kt_datatable_horizontal_scroll"
                                    ...
                                    <tbody class="fw-semibold text-gray-600">
                                        <tr v-if="plates.data.length === 0">
                                            <td colspan="5" class="text-center py-10">
                                                <img src="/assets/media/illustrations/empty.png"
                                                    alt="Tidak ada data" style="max-height: 260px;">
                                                <h2 class="text-gray-800">Jenis Plat Tidak Ditemukan</h2>
                                                <p class="text-gray-600 fs-4">Belum ada jenis plat atau pencarian tidak sesuai.</p>
                                            </td>
                                        </tr>
                                        <tr v-else v-for="(plate, index) in plates.data" :key="index">
                                            <td class="ps-8">
                                                {{ ++index + (plates . current_page - 1) * plates . per_page }}
                                            </td>
                                            <td class="text-gray-800">{{ plate . type }}</td>
                                            <td>{{ plate . description ?? '-' }}</td>
                                            <td>
                                                <span  class="badge badge-lg"
                                                    :class="{
                                                        'bg-mysuccess text-mysuccess': plate.status === 'Y',
                                                        'bg-mydanger text-mydanger': plate.status === 'N'
                                                    }">
                                                    {{ plate.status === 'Y' ? 'Aktif' : 'Nonaktif' }}
                                                </span>
                                            </td>
                                            <td class="text-end pe-8">
                                                <button @click="editPlate(plate)"
                                                    class="btn btn-sm btn-icon btn-mysecondary me-2">
                                                    <i class="ri-edit-2-fill fs-3"></i></button>
                                                <button @click.prevent="destroy(plate.id)"
                            ...
                            <Pagination :links="plates.links" align="end" class="px-8" />
                        </div>
                    </div>
                </div>
            </div>
                                ...
                                <div class="card-body p-6">
                                    <div class="mb-5 fv-row">
                                        ...
                                        <input type="text" class="form-control fs-5" v-model="form.type"
                                            required placeholder="Masukkan jenis plat disini..."
        ...
        setup() {
            ...
            const submit = () => {
                ...
                if (isEditMode.value && editing.value) {
                    // UPDATE
                    router.put(`/admin/plates/${editing.value.id}`, payload, {
                        onSuccess: () => {
                            Swal.fire({
                                title: 'Berhasil!',
                                text: 'Jenis Plat Diperbarui!',
                                icon: 'success',
                    ...
                    // STORE
                    router.post('/admin/plates', payload, {
                        onSuccess: () => {
                            Swal.fire({
                                title: 'Berhasil!',
                                text: 'Jenis Plat Berhasil Disimpan!',
                                icon: 'success',
                                showConfirmButton: false,
                                buttonsStyling: false,
                                timer: 2000,
            ...
            const editPlate = (plate) => {
                ...
                form.type = plate.type;
                form.description = plate.description;
                form.status = plate.status;
            ...
            const destroy = (id) => {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        router.delete(`/admin/plates/${id}`);
                        Swal.fire({
                            title: 'Dihapus!',
                            text: 'Jenis Plat Berhasil Dihapus!',
                            icon: 'success',
                            timer: 2000,
                            showConfirmButton: false,
                            buttonsStyling: false,
